		...
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="position:inherit;">
			...
			<div class="table-responsive col-xs-9 col-sm-9 col-md-9 col-lg-9" style="position:relative; padding-bottom:30px;">
				...
				<div class="col-lg-8 mtr_a big_font_size">
					<span><a href="#weekday" data-toggle="tab" aria-expanded="true" @click="searchMonthEmployee">月</a></span>
					|
...
<script>
	...
	export default {
		...
		methods: {
			...
			changeContent() {
				var index = 0;
				this.ryScheduleList = new Array();
				for (var i = 1; i < 32; i++) {
					this.ryScheduleList[index] = '公休_' + i;
					index++;
				}
				//console.log("ryScheduleList:" + this.ryScheduleList);

				for (var x = 0; x < this.ryScheduleList.length; x++) {
					 var item = $("#day li");
					 console.log("item:"+item.length);
					 for(var v = 0;v < item.length; v++){
						 console.log("v："+item.text());
					 }
					
					// $("#day_" ).text() =;
					//$("#day_"+ (x + 1)).text(this.ryScheduleList[x]);
				}
			},
			init() {
				$(function(){
								...
								for (var n = 0; n < 7; n++) { //列
									if ((7 * m + n) < codeFirst || (7 * m + n) >= (codeFirst + monthDays[codeMonth])) { //某月日历中不存在的日期
										...
										if ((7 * m + n + 1 - codeFirst == codeDay) && (((7 * m + n) % 7 == 0) || ((7 * m + n) % 7 == 6))) { //当天是周末
											kalendar_html += "<li class='todayWeekend'><span class='xiu'>1</span>" + (7 * m + n + 1 - codeFirst) + "</li>";
										}
										/*else if(((7*m+n)%7 == 0) || ((7*m+n)%7 == 6)){//仅是周末
																						kalendar_html += "<li class='weekend'><span class='xiu>1</span>"+(7*m+n+1-codeFirst)+"</li>";
																					}*/
										else if (7 * m + n + 1 - codeFirst == codeDay) { //仅是当天
											kalendar_html += "<li class='today'><span class='xiu' id='day_" + (7 * m + n + 1 - codeFirst) +
												"'>1</span>" + (7 * m + n + 1 - codeFirst) + "</li>";
										} else { //其他日期
											kalendar_html += "<li><span class='xiu' id ='day_" + (7 * m + n + 1 - codeFirst) + "'>公休</span>" + (7 * m +
												n + 1 - codeFirst) + "</li>";
										}
									}
								}
